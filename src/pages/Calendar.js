...
function Calendar({
  ...
  const fetchWeather = async () => {
    const response = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Ecuador?unitGroup=metric&key=5LQWAFGK7VFALAM3SWURFYY8L&contentType=json`
    );
  ...
  for (let i = firstDayOfMonth - 1; i >= 0; i--) {
    ...
    calendarDays.push(
      <div
        key={`prev-${i}`}
        className="empty-day"
        onClick={() => navigateToMonth(prevDateString)}>
        ...
        {prevReminders.slice(0, 1).map((reminder, index) => (
          <div
            ...
            {reminder.text} - {reminder.city} H: {reminder.time} (
            {reminder.weather})
          </div>
        ))}
        {prevReminders.length >= 2 && (
          <div
            ...
            onClick={() => handleDayClick(prevReminders)}>
            +{prevReminders.length - 1} más
          </div>
        )}
      </div>
    );
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarDays.push(
      <div
        key={day}
        className="day"
        onClick={dayReminders.length === 0 ? handleCancelEdit : undefined}>
        ...
        {dayReminders.length >= 2 && (
          <div
            className="more-reminders"
            onClick={() => handleDayClick(dayReminders)}>
            +{dayReminders.length - 1} más
          </div>
        )}
      </div>
    );
  ...
  for (let i = 1; i <= remainingDays; i++) {
    ...
    calendarDays.push(
      <div
        key={`next-${i}`}
        className="empty-day"
        onClick={() => navigateToMonth(nextDateString)}>
        ...
        {nextReminders.length >= 2 && (
          <div
            className="more-reminders"
            onClick={() => handleDayClick(nextReminders)}>
            +{nextReminders.length - 1} más
          </div>
        )}
      </div>
    );
    ...
    <div className="containerCalendar">
      ...
      <form onSubmit={handleAddReminder}>
        <input
          ...
          defaultValue={editReminder ? editReminder.text : ""}
          required
        />
        <input
          name="date"
          type="date"
          defaultValue={editReminder ? editReminder.date : ""}
          required
        />
        <input
          name="time"
          type="time"
          defaultValue={editReminder ? editReminder.time : ""}
          required
        />
        <input
          name="city"
          type="text"
          placeholder="Ciudad"
          defaultValue={editReminder ? editReminder.city : ""}
          required
        />
        <button type="submit">
          {editReminder ? "Actualizar Recordatorio" : "Agregar Recordatorio"}
        </button>
      </form>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setEditReminder: (reminder) =>
    dispatch({ type: "SET_EDIT_REMINDER", payload: reminder }),
